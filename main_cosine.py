...
# 打開一個 CSV 檔案，進行寫入操作
with open('formal_cosine_accuracy_output.csv', 'w', newline='') as csvfile:
    ...
    for i in tqdm(range(len(df)), desc="計算相似度", unit="row"):
        ...
        current_answer = ans.iloc[i, 2]  # 第 202 欄（答案）
        ###########################################################################
        # 更改euclidean，cosine
        ...
        ###########################################################################
        # 找出前 10 相似的行（排除自己）
        top_10_indices = cosine_similarities.nlargest(11).index[1:]  # 取前 11 名，排除自己
...
data_name.to_csv('formal_cisine_data_name.txt', index=False, encoding='utf-8')
# 計算總準確率
...
with open('formal_accuracy.txt', 'a') as file:
    file.write(f' cosine_similarities 總準確率: {overall_accuracy:.2%}\n')